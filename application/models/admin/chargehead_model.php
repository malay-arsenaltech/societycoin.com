...
class Chargehead_model extends CI_Model {
    ...
    public function get_selected_data($society_id) {
        ...
        $data = $this->db->select("ci_society_chargehead.*,ci_chargehead.charge_head_name,ci_chargehead.id as charge_head_id")->join("ci_society_chargehead", "ci_society_chargehead.chargehead_id = ci_chargehead.id")->where("society_id", $society_id)->where("status", "1")->from("ci_chargehead")->get();
        return $data->result();
    }
